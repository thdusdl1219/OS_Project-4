...
static bool
ppid_same (const struct list_elem *a_, const struct list_elem *b_ UNUSED, void *aux)
{
	  const struct pid_elem *a = list_entry(a_, struct pid_elem, elem);
		return a ->pid == (int )aux;
}
/* Deschedules the current thread and destroys it.  Never
   returns to the caller. */

/*static bool
tid_same (const struct list_elem *a_, const struct list_elem *b_ UNUSED, void *aux)
{
	const struct exe_elem *a = list_entry(a_, struct exe_elem, elem);
	return a->pid == (int)aux;
}*/
...
thread_exit (void) 
{
  ...
  intr_disable ();
	...
	if(p != NULL)
	{
		p->exit_s = thread_current()->exit_status;
		p->die = true;
		sema_up(p->lock);
	}

	if(load_lock.holder == thread_current())
	{
		lock_release(&load_lock);
	}

	if(open_lock.holder == thread_current())
	{
